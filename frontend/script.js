...
function initTrainingWorkflow() {
    ...
    function updateParameterForm(algorithm) {
        ...
        switch(algorithm) {
            ...
            case 'svm':
                ...
                break;
            default:
                formHTML = '<p class="no-algo-message">Please select an algorithm first</p>';
        }
    ...
    trainButton.addEventListener('click', async (event) => {
        ...
        try {
            ...
            trainButton.innerHTML = '<div class="processing-spinner"></div> Training...';
            
            // Show training progress notification
            const trainingNotificationId = Date.now();
            const trainingNotification = showNotification('Training model, please wait...', 'info', 0); // No auto-dismiss
            ...
            if (selectedAlgorithm === 'knn' || selectedAlgorithm === 'k_means_clustering') {
            ...
            displayMetricsByAlgorithm(selectedAlgorithm, processedMetrics, file, modelResults);
            ...
            const existingTrainingNotifications = document.querySelectorAll('.notification');
            existingTrainingNotifications.forEach(notification => {
                if (notification.querySelector('p').textContent === 'Training model, please wait...') {
    ...
    function processMetricsByAlgorithm(algorithm, metrics) {
        ...
        const processed = {...defaultMetrics, parameters: metrics.parameters || {}};
    ...
    function displayMetricsByAlgorithm(algorithm, metrics, file, container) {
        ...
        const header = `
            <div class="result-card">
                <h3>Training Results for ${formatAlgorithmDisplayName(algorithm)}</h3>
        `;
        
        const footer = `
                <div class="training-time">
                    <p>Trained on ${file ? `${file.name} (${formatFileSize(file.size)})` : 'dataset'}</p>
                    <p>Training completed at ${new Date().toLocaleTimeString()}</p>
                    ${metrics.time_ms ? `<p>Training time: ${(metrics.time_ms / 1000).toFixed(2)}s</p>` : ''}
                    ${metrics.memory_kb ? `<p>Memory usage: ${formatFileSize(metrics.memory_kb * 1024)}</p>` : ''}
                </div>
            </div>
        `;
        ...
        if (metrics.r2 && algorithm === 'linear-regression') {
            ...
            metricsHTML = `
                <div class="metrics-grid single-metric">
                    <div class="metric large-metric" style="animation: fadeIn 0.3s ease forwards; opacity: 0;">
                        ...
                        <span class="metric-value">${metrics.inertia.toFixed(2)}</span>
                        <span class="metric-description">Sum of squared distances to the nearest centroid</span>
                    </div>
                </div>
            `;
        } else if (metrics.accuracy || metrics.precision || metrics.recall || metrics.f1_score) {
                ...
                <div class="metrics-grid">
                    ${metrics.accuracy ? `
                        <div class="metric" style="animation: fadeIn 0.3s ease forwards; animation-delay: 0.1s; opacity: 0;">
                            ...
                            <span class="metric-value">${metrics.accuracy.toFixed(2)}%</span>
                        </div>
                    ` : ''}
                    ${metrics.precision ? `
                        <div class="metric" style="animation: fadeIn 0.3s ease forwards; animation-delay: 0.2s; opacity: 0;">
                            <span class="metric-label">Precision</span>
                            <span class="metric-value">${metrics.precision.toFixed(2)}%</span>
                        </div>
                    ` : ''}
                    ${metrics.recall ? `
                        <div class="metric" style="animation: fadeIn 0.3s ease forwards; animation-delay: 0.3s; opacity: 0;">
                            <span class="metric-label">Recall</span>
                            <span class="metric-value">${metrics.recall.toFixed(2)}%</span>
                        </div>
                    ` : ''}
                    ${metrics.f1_score ? `
                        <div class="metric" style="animation: fadeIn 0.3s ease forwards; animation-delay: 0.4s; opacity: 0;">
                            <span class="metric-label">F1 Score</span>
                            <span class="metric-value">${metrics.f1_score.toFixed(2)}%</span>
                        </div>
                    ` : ''}
                </div>
            `;
            ...
            const metricsToShow = Object.entries(metrics).filter(([key]) => 
                !['parameters', 'time_ms', 'memory_kb'].includes(key)
            );
            
            if (metricsToShow.length > 0) {
                metricsHTML = `
                    <div class="metrics-grid">
                        ${metricsToShow.map(([key, value], index) => `
                            <div class="metric" style="animation: fadeIn 0.3s ease forwards; animation-delay: ${0.1 * index}s; opacity: 0;">
                                <span class="metric-label">${formatMetricName(key)}</span>
                                <span class="metric-value">${typeof value === 'number' ? value.toFixed(2) : value}${key !== 'inertia' ? '%' : ''}</span>
                            </div>
                        `).join('')}
                    </div>
                `;
    ...
    function formatParameterName(name) {
        switch(name) {
            case 'lr': return 'Learning Rate';
            case 'epochs': return 'Epochs';
            case 'k': return 'K Value';
            case 'C': return 'Regularization (C)';
            default:
                return name
                    .split('_')
                    .map(word => word.charAt(0).toUpperCase() + word.slice(1))
    ...
    function updateTrainingHistory(history) {
        ...
        historyList.innerHTML = history.map((record, index) => {
            ...
            if (record.algorithm === 'linear-regression' && record.metrics.r2) {
            ...
            } else if (record.algorithm === 'k_means_clustering' && record.metrics.inertia) {
                ...
                const metricKey = Object.keys(record.metrics).find(key => 
                    !['parameters', 'time_ms', 'memory_kb'].includes(key)
                );
                if (metricKey) {
                    const metricValue = record.metrics[metricKey];
                    metricDisplay = `${formatMetricName(metricKey)}: ${typeof metricValue === 'number' ? metricValue.toFixed(2) : metricValue}${metricKey !== 'inertia' ? '%' : ''}`;
                } else {
            ...
            return `
                <div class="history-item" style="animation: fadeIn 0.3s ease forwards; animation-delay: ${0.1 * index}s; opacity: 0;">
                    <div class="history-icon ${record.algorithm}">
                        <i class="fas fa-${getAlgorithmIcon(record.algorithm)}"></i>
                    </div>
                    <div class="history-details">
                        <div class="history-name">${formatAlgorithmDisplayName(record.algorithm)} #${index + 1}</div>
    ...
    function updateVisualizationWithResults(algorithm, metrics) {
        ...
        console.log(`Visualization would be updated for ${algorithm} with metrics:`, metrics);
    }
    ...
    window.showModelDetails = function(index) {
        ...
        // Display metrics based on algorithm type
        displayMetricsByAlgorithm(record.algorithm, record.metrics, null, modelResults);
...
function initFileUpload() {
    ...
    fileInput.addEventListener('change', async () => {
        ...
        try {
            ...
            const iconClass = extension === 'csv' ? 'fa-file-csv' : 
                             extension === 'json' ? 'fa-file-code' : 
                             extension === 'xlsx' ? 'fa-file-excel' : 'fa-file';
...
function parseCSV(content) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (values.length !== header.length) {
            console.warn(`Line ${i} has ${values.length} values instead of ${header.length}`);
            continue;
        }
    ...
    infoContainer.innerHTML = `
                ...
                <table class="preview-table">
                    ...
                    <tbody>
                        ${data.data.slice(0, 5).map(row => `
                            <tr>
                                ${data.header.map(col => `<td>${row[col]}</td>`).join('')}
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
        ...
        <div class="column-stats">
            ...
            <div class="column-stats-grid">
                ${stats.columnStats.slice(0, 4).map(col => `
                    <div class="column-stat-card">
                        <div class="column-name">${col.name} <span class="column-type">${col.type}</span></div>
                        <div class="stat-values">
                            ${col.type === 'numeric' ? `
                                <div class="stat-item">
                                    <span>Min: </span><span>${col.min.toFixed(2)}</span>
                                </div>
                                <div class="stat-item">
                                    <span>Max: </span><span>${col.max.toFixed(2)}</span>
                                </div>
                                <div class="stat-item">
                                    <span>Mean: </span><span>${col.mean.toFixed(2)}</span>
                                </div>
                            ` : `
                                <div class="stat-item">
